...
const DataModule = {
    ...
    // Update the saveProduct function to work with UUIDs
async saveProduct(product) {
    ...
    try {
        ...
        // Check if product.id exists and is a valid UUID
        if (product.id && product.id.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)) {
            const { data, error } = await supabase
                .from('products')
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
            ...
            // Generate a temporary UUID for offline products
            if (!product.id || !product.id.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)) {
...
async function syncPurchase(operation) {
    try {
        ...
        const purchaseData = { ...operation.data };
...
async function showApp() {
    ...
    try {
        ...
        // Check for stock alerts after loading data
        checkAndGenerateAlerts();
...
async function refreshAllData() {
    try {
        ...
        // Check for stock alerts after refreshing data
        checkAndGenerateAlerts();
...
function loadStockAlerts() {
    ...
    try {
        ...
        const allAlerts = checkAndGenerateAlerts();
        
        // Update alert summary badges
        const expiredCount = allAlerts.filter(a => a.message.includes('expired')).length;
        const expiringSoonCount = allAlerts.filter(a => a.message.includes('expires')).length;
        const lowStockCount = allAlerts.filter(a => a.message.includes('items left') || a.message.includes('out of stock')).length;
        ...
        if (allAlerts.length === 0) {
            ...
            allAlerts.forEach(alert => {
                ...
                } else if (alert.message.includes('expires')) {
                ...
                alertDiv.innerHTML = `
                    ...
                    <div class="alert-content">
                        ...
                        <div class="alert-time">${alert.message.includes('expires') ? `Expires on: ${formatDate(alert.expiryDate)}` : `Current stock: ${alert.currentStock}`}</div>
                    </div>
